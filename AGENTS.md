...
Codex History Viewer is a Tauri-based desktop application that allows users to browse and analyze conversation history from multiple AI coding assistants: Codex (`~/.Codex`), Codex CLI (`~/.codex`), and OpenCode (`~/.local/share/opencode/`).
...
```
Codex: ~/.Codex/projects/[project]/*.jsonl  ─┐
...
- **Main Commands** (in `src-tauri/src/lib.rs`):
  - `get_claude_folder_path` - Locates user's `.Codex` directory
  - `scan_projects` - Scans for all Codex projects
...
- **Data Structure**: Reads JSONL files containing conversation history from `~/.Codex/projects/`
...
{
  "common.appName": "Codex History Viewer",
...
- The app expects Codex conversation data in `~/.Codex/projects/[project-name]/*.jsonl`
...
- The app uses Tailwind CSS with custom Codex brand colors defined in `tailwind.config.js`
...
## Codex Directory Structure Analysis
...
- **2025 Content Types Support (December 2025)**:
  - Added support for new content types from Codex API 2025 updates
...
- **Data Structure & Type Correction (June 2025)**:
  - Performed a deep analysis of `.jsonl` log files in the `~/.Codex` directory to verify the exact data structure.
